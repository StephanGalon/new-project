...
// check whether a given temperatures is less than 0 and the other is greater than 100.
...
//check whether two given integer values are in the range 20..50 inclusive. Return true if 1 or other is in the said range otherwise false.
...
// check which number nearest to the value 100 among two given integers. Return 0 if the two numbers are equal.
...
// find the larger value from two positive integer values that is in the range 20..30 inclusive, or return 0 if neither is in that range.
fun oneValueIn(x: Int, y: Int) :Int{
//    return if((x >= 20 && x <= 30 && y >= 20 && y <= 30)) if (x >= y) x else y
//        else if (x >= 20 && x <= 30) x
//        else if (y >= 20 && y <= 30) y
//    else 0
    return if(x >= y && x in 20..30 && y in 20..30) x else y
...
//count the number of two 5's are next to each other in an array of integers. Also count the situation where the second 5 is actually a 6.
...
//check if a triple is presents in an array of integers or not. If a value appears three times in a row in an array it is called a triple.
...
//test whether a given non-negative number is a multiple of 13 or it is one more than a multiple of 13.
...
// check whether a given non-negative number is a multiple of 3 or 7, but not both.
...
//  check whether a given number is within 2 of a multiple of 10.
...
// check whether it is possible to add two integers to get the third integer from three given integers.
...
//check whether y is greater than x, and z is greater than y from three given integers x,y,z.
...
//  check whether two or more non-negative given integers have the same rightmost digit.
...
//compute the sum of the three given integers. However, if any of the values is in the range 10..20 inclusive then that value counts as 0, except 13 and 17.
...
// check two given integers and return the value whichever value is nearest to 13 without going over. Return 0 if both numbers go over.
...
//the difference between medium and large is same.
...
//check a given array of integers of length 1 or more and return true if the first element and the last element are equal in the given array.
...
//check two given arrays of integers of length 1 or more and return true if they have the same first element or they have the same last element.
...
//compute the sum of the elements of a given  array of integers.
...
// create a new array taking the first and last elements of a given array of integers and length one or more.
...
// check whether a given array of integers of length 2, contains 15 or 20.
...
// check whether a given array of integers of length 2, does not contain 15 or 20.
...
// check a given array of integers and return true if the array contains 10 or 20 twice. The length of the array will be 0, 1, or 2.
...
//check a given array of integers of length 3 and create a  new array. If there is a 5 in the given array immediately followed by a 7 then set 7 to 1.
...
//compute the sum of the two given arrays of integers of length 3 and find the array which has the largest sum.
...
//create a new array from two given array of integers, each length 3.
...
// create a new array of length 3 from a given array (length atleast 3) containing the elements from the middle of the array.
